      ...
      <div className="min-h-screen bg-white font-sans text-slate-900">
        ...
        <main className="max-w-7xl mx-auto px-6 py-20 grid lg:grid-cols-2 gap-12 items-center">
          ...
          <motion.div 
            ...
            className="relative"
          >
            <div className="bg-slate-900 rounded-[2.5rem] p-8 shadow-2xl relative z-10 overflow-hidden">
            ...
            <div className="absolute -bottom-6 -right-6 w-32 h-32 bg-indigo-600 rounded-3xl -z-0 rotate-12 opacity-20 blur-2xl" />
          </motion.div>
        </main>
      </div>
    );
        ...
        {activeTab === "dashboard" && (
          ...
          >
            <header className="mb-10 flex justify-between items-end">
              <div>
                <h2 className="text-3xl font-bold tracking-tight mb-2">Welcome back, {user.name.split(' ')[0]}!</h2>
                <p className="text-slate-500">Here's how your LinkedIn profile is performing.</p>
              </div>
              {!analysis && (
                <button 
                  onClick={() => setActiveTab("generator")}
        ...
        {activeTab === "generator" && (
          ...
          >
            <header className="mb-10">
              ...
              <p className="text-slate-500">Generate high-impact LinkedIn posts tailored to your profile.</p>
            </header>

            <div className="grid lg:grid-cols-3 gap-8">
              <div className="lg:col-span-1 space-y-6">
                <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm">
                  <label className="block text-sm font-bold text-slate-400 uppercase tracking-wider mb-3">Profile Data</label>
                  <textarea 
                    value={profileText}
                    onChange={(e) => setProfileText(e.target.value)}
                    placeholder="Paste your LinkedIn profile text or PDF export content here..."
                    className="w-full h-48 p-4 bg-slate-50 rounded-2xl border border-slate-100 focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none text-sm resize-none"
                  />
                  <button 
                    onClick={handleAnalyze}
                    disabled={isAnalyzing || !profileText}
                    className="w-full mt-4 bg-indigo-50 text-indigo-700 py-3 rounded-xl font-bold hover:bg-indigo-100 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                  >
                ...
                <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm">
                  <label className="block text-sm font-bold text-slate-400 uppercase tracking-wider mb-3">Target Role</label>
                  <input 
                    type="text"
                    value={targetRole}
